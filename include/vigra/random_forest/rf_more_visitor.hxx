 ...
 *  vigra
 *
 *  Created by Luca Fiaschi on 5/3/11.
...
class IndexVisitorTrainingOld: public VisitorBase
{
...
public:
	...
	typedef std::vector<int> Point; //structure for the point in the matrxi
		...
		if (tree.isLeafNode(split.createNode().typeID()))
		{
			...
			while (it != parent.end())
			{
				...
				if (labels(*it, 0) != 0)
				{
					...
					if (p.size() != 2)
						throw std::runtime_error("only 2D supported");
			...
			prob = count_pos / double(parent.size());
	...
	* return a double value.  The value of the first
	...
	void save(std::string filen, std::string pathn)
	{
		...
		for (KeyIt it = mapping.begin(); it != mapping.end(); it++)
		{
			...
			//std::cout << "arr-size " << arr_size << std::endl;
			std::vector<Point>& temp_vec = (*it).second;
...
public:
	...
	std::map<std::pair<tree_index, leaf_index>, std::vector<Point> > mapping; //map tree and leaf with index of the sample
	std::map<std::pair<tree_index, leaf_index>, double> mapping_to_prob;
	IndexVisitorTraining(int dim_ ) :
		current_tree(0), current_sample(0), dim(dim_)
	{
	}

	IndexVisitorTraining( ) :
			current_tree(0), current_sample(0), dim(2)
		{
		}
...
public:
	...
	IndexVisitorTrainingRegressionForest(int dim_ ) :
		current_tree(0), current_sample(0), dim(dim_)
	{
	}

	IndexVisitorTrainingRegressionForest( ) :
			current_tree(0), current_sample(0), dim(2)
		{
		}
